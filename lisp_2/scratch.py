...
if __name__ == "__main__":
    ...
    # tokens = tokenize("(if (or (> 3 4) (> 5 6)) 1 0)")
    # parsed = parse(tokens)
    # evaluate(parsed, testing_frame)
    # tokens = tokenize("(define not (lambda (x y z) (+ x (* y z))))")
    # parsed = parse(tokens)
    # evaluate(parsed, testing_frame)

    # tokens = tokenize("(not 1 2 3)")
    # parsed = parse(tokens)
    # evaluate(parsed, testing_frame)
    ...
    parsed = parse(tokens)
    a = evaluate(parsed, testing_frame)

    tokens = tokenize("(define x (list 0 1 2 3 4))")
    parsed = parse(tokens)
    b = evaluate(parsed, testing_frame)

    tokens = tokenize("(list-ref x 2)")
    parsed = parse(tokens)
    c = evaluate(parsed, testing_frame)

    tokens = tokenize("(map - x)")
    parsed = parse(tokens)
    d = evaluate(parsed, testing_frame)
    # cop = shallow_copy(d)
